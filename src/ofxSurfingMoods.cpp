...
void ofxSurfingMoods::setup()//default sizes
{
	...
	{
#ifdef USE_PRESETS_STARTS_AT_1_NOT_0//we use presets from 1 to 8
		presets_A[i] = 1;
		presets_B[i] = 1;
		presets_C[i] = 1;
#else
	...
	if (!isLoaded)
	{
		ofLogError("ofxSurfingMoods") << "ofTrueTypeFont FONT FILE '" << myTTF << "' NOT FOUND!";
	}
...
void ofxSurfingMoods::setup_GUI_Customize()
{
	...
	(group_USER->getIntSlider("COUNT DURATION"))->setConfig(jHigh);
	...
	{
		ofLogError("ofxSurfingMoods") << "setup_GUI_Customize() FILE '" << myTTF_Gui << "' NOT FOUND!";
	}
	...
	//theme
	group_Advanced->loadTheme("theme/theme_bleurgh.json");
	group_USER->loadTheme("theme/theme_bleurgh.json");
	//group_RANGES;
	//group_TARGETS;
	//group_CLOCK;
}
...
{
	ofLogVerbose("ofxSurfingMoods") << "refresh_MOOD_Color";
	...
	//ofLogVerbose("ofxSurfingMoods") << "colorLabel: " << ofToString(colorLabel);

	//-

	//colorize
	std::string cStr = ofxGui::colorToString(colorLabel);
	//ofLogVerbose("ofxSurfingMoods") << "mood color: " << cStr;
	//"rgba(0,255,255,0.4)"
...
void ofxSurfingMoods::setup_Params()
{
	...
	PRESET_A_Selected.set("PRESET A", iGap, iStart, NUM_PRESETS_A - iGap);
	PRESET_B_Selected.set("PRESET B", iGap, iStart, NUM_PRESETS_B - iGap);
	PRESET_C_Selected.set("PRESET C", iGap, iStart, NUM_PRESETS_C - iGap);
	...
	COUNT_Duration.set("COUNT DURATION", 4, 1, 16);
	COUNTER_step_FromOne.set("COUNTER", 0, 0, COUNT_Duration);

	bRandomize_Bank.set("RANDOMIZE BANK", false);
	bRandomize_Bank.setSerializable(false);
	bReset_Bank.set("RESET BANK", false);
	bReset_Bank.setSerializable(false);
	bResetSort_Bank.set("RESET SORT BANK", false);
...
{
	ofLogNotice("ofxSurfingMoods") << "load_range";

	if ((r < 0) || (r >= NUM_RANGES))
	{
		ofLogError("ofxSurfingMoods") << "load_range. OUT OF RANGE";
	}
	else
	{
		////clamp ranges
...
{
	ofLogNotice("ofxSurfingMoods") << "save_range";

	if ((r < 0) || (r >= NUM_RANGES))
	{
		ofLogError("ofxSurfingMoods") << "save_range. OUT OF RANGE";
	}
	else
	{
		////clamp ranges
...
//	ofLogNotice("ofxSurfingMoods") << "updateLabels";
//
//	string pad;
//	//pad = " ";
//	//pad = "\t";
...
{
	ofLogNotice("ofxSurfingMoods") << "stopGen";
...
{
	ofLogNotice("ofxSurfingMoods") << "resetClock";
...
{
	ofLogNotice("ofxSurfingMoods") << "resetBank";
	...
	for (int p = 0; p < NUM_TARGETS; p++)
	{
		if (SORT_RELATIVE)
		{
			...
			iDif = 1;
#endif
			presets_A[p] = MIN(p, NUM_PRESETS_A - iDif);
			presets_B[p] = MIN(p, NUM_PRESETS_B - iDif);
			presets_C[p] = MIN(p, NUM_PRESETS_C - iDif);
		}
		else
		{
			int iStart;
			...
			{
				presets_A[p] = (int)ofRandom(iStart, NUM_PRESETS_A - iGap + 1);
				presets_B[p] = (int)ofRandom(iStart, NUM_PRESETS_B - iGap + 1);
				presets_C[p] = (int)ofRandom(iStart, NUM_PRESETS_C - iGap + 1);
			}
		}
	}
...
void ofxSurfingMoods::setPosition(int _x, int _y)
{
	...
	{
		setGuiAdvancedPositon(_x + 210, _y);
	}
...
//--------------------------------------------------------------
void ofxSurfingMoods::EngineSwitch()
{
	...
	if (COUNTER_step == 0)
	{
		...
		ofLogNotice("ofxSurfingMoods") << "RANGE_Selected: " << RANGE_Selected;
	}

	//do randomize
	TARGET_Selected = ofRandom(Range_Min, Range_Max + 1);
	ofLogNotice("ofxSurfingMoods") << "TARGET RANDOM: " << TARGET_Selected;

	//-

	//3. mode loop:  0-1-2 .. 0-1-2 ..
...
{
	ofLogVerbose("ofxSurfingMoods") << "clone targets";
...
{
	ofLogVerbose("ofxSurfingMoods") << "timer_Range_Complete";
...
{
	ofLogVerbose("ofxSurfingMoods") << "timer_Range_Started";
}
...
void ofxSurfingMoods::saveBanks(string path)
{
	...
	//if ((int)MAX_ITEMS > 0 && (int)MAX_ITEMS < NUM_TARGETS)
	{
		...
		for (int i = 0; i < NUM_TARGETS; i++)
		{
...
#ifdef USE_PRESETS_STARTS_AT_1_NOT_0//we use presets from 1 to 8
			//clamp min
			int prst_A = (int)MAX(presets_A[i], 1);
			int prst_B = (int)MAX(presets_B[i], 1);
			int prst_C = (int)MAX(presets_C[i], 1);
			//clamp max
			...
			//clamp max
			prst_A = (int)MIN(prst_A, NUM_PRESETS_A - 1);
			prst_B = (int)MIN(prst_B, NUM_PRESETS_B - 1);
			prst_C = (int)MIN(prst_C, NUM_PRESETS_C - 1);
#endif
			...
			ofLogNotice("ofxSurfingMoods") << "presets: " << pt;
		}
		bool bSaved = ofSavePrettyJson(path + fileBank, js_targets);

		if (bSaved)
		{
			ofLogNotice("ofxSurfingMoods") << "ofSaveJson: " << js_targets;
		}
		else
		{
			ofLogError("ofxSurfingMoods") << "CAN'T SAVE FILES INTO: " << path;
		}
	}
	//else
	//{
	//ofLogError("ofxSurfingMoods") << "OUT OF MAX_ITEMS";
	//}
...
void ofxSurfingMoods::saveSettings(string path)
{
	...
	ofLogNotice("ofxSurfingMoods") << "saveSettings: " << filePath;
...
void ofxSurfingMoods::loadBanks(string path)
{
	...
	if (file.exists())
	{
		...
		//cout << js_targets;
		ofLogNotice("ofxSurfingMoods") << "LOADED FILE: " << pathBank;
		ofLogNotice("ofxSurfingMoods") << js_targets;

		ofLogWarning("ofxSurfingMoods") << pathBank + " json file must be present and formatted as expected!";
		...
		for (auto &js_tar : js_targets)
		{
			...
			{
				ofLogVerbose("ofxSurfingMoods") << endl << js_tar;
				//cout << js_tar << endl;
...
#ifdef USE_PRESETS_STARTS_AT_1_NOT_0//we use presets from 1 to 8
				//clamp min
				prst_A = (int)MAX(prst_A, 1);
				prst_B = (int)MAX(prst_B, 1);
				prst_C = (int)MAX(prst_C, 1);
				//clamp max
				...
				//clamp max
				presets_A[p] = (int)MIN(prst_A, NUM_PRESETS_A - 1);
				presets_B[p] = (int)MIN(prst_B, NUM_PRESETS_B - 1);
				presets_C[p] = (int)MIN(prst_C, NUM_PRESETS_C - 1);
#endif

				ofLogVerbose("ofxSurfingMoods") << presets_A[p];
				ofLogVerbose("ofxSurfingMoods") << presets_B[p];
				ofLogVerbose("ofxSurfingMoods") << presets_C[p];
				p++;
			}
		}
	}
	...
	{
		ofLogError("ofxSurfingMoods") << "loadSettings '" << pathBank << "' NOT FOUND!";
	}
}
...
{
	ofLogNotice("ofxSurfingMoods") << "loadSettings";
	...
	if (bLoaded)
	{
		ofLogNotice("ofxSurfingMoods") << "LOADED: " << filePath;
		ofLogNotice("ofxSurfingMoods") << endl << settings_GEN.toString();

		ofDeserialize(settings_GEN, params_STORE);
	}
	else
	{
		ofLogError("ofxSurfingMoods") << "FILE NOT FOUND: " << filePath;
	}
...
void ofxSurfingMoods::Changed_Params_Listeners(ofAbstractParameter &e)
{
	if (!BLOCK_CALLBACK_Feedback)
	{
		...
		if (WIDGET != "COMPLETE")
			ofLogVerbose("ofxSurfingMoods") << "Changed_Params_Listeners '" << WIDGET << "': " << e;
		...
		{
			ofLogNotice("ofxSurfingMoods") << "CHANGED TARGET_Selected:" << TARGET_Selected;
...
void ofxSurfingMoods::Changed_Ranges(ofAbstractParameter &e)
{
	if (!BLOCK_CALLBACK_Feedback)
	{
		...
		ofLogVerbose("ofxSurfingMoods") << "Changed_Ranges '" << name << "': " << e;

		if (name == "RANGE")
		{
			ofLogNotice("ofxSurfingMoods") << "CHANGED RANGE_Selected:" << RANGE_Selected;
			...
			{
				ofLogNotice("ofxSurfingMoods") << "RANGE_Selected: " << RANGE_Selected.get();
